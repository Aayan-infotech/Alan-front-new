...
//       try {
//         const response = await axios.get(`http://18.221.196.222:7878/api/products/getProductsbyid/${productIdfordet}`);
...
const DimensionsProductDoors = () => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        const res = await axios.get(`http://18.221.196.222:7878`);
        setProduct(res.data);
      } catch (err) {
  ...
  const handleAddData = async (section) => {
    ...
    try {
      const res = await fetch(`http://18.221.196.222:7878`, {
        method: 'POST',
  ...
  const handleDeleteData = async (section, id, index) => {
    try {
      await fetch(`http://18.221.196.222:7878`, { method: 'DELETE' });
