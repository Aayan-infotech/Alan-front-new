...
const DimensionsProduct = () => {
    ...
    const fetchAllData = async () => {
      ...
      for (const [section, api] of Object.entries(apiEndpoints)) {
        try {
          ...
          const res = await fetch(`http://18.221.196.222:7878/api/dims/type/${section}/ProductID/${Product_id}`);
  ...
  const handleAddData = async (section) => {
    ...
    try {
      const res = await fetch(`http://18.221.196.222:7878/api/${apiEndpoints[section]}`, {
        method: 'POST',
  ...
  const handleDeleteData = async (section, id, index) => {
    try {
      await fetch(`http://18.221.196.222:7878/api/${apiEndpoints[section]}/${id}`, { method: 'DELETE' });
  ...
  const fetchAllData = async () => {
    ...
    for (const [section, api] of Object.entries(apiEndpoints)) {
      try {
        const res = await fetch(`http://18.221.196.222:7878/api/${api}`);
        const data = await res.json();
