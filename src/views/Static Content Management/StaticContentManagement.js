...
const StaticContentManagement = () => {
  ...
  const fetchContent = async (section) => {
    try {
      const response = await axios.get(`http://18.209.91.97:7778/api/StaticContent/${section}`);
  ...
  const saveContent = async () => {
    try {
      await axios.post('http://18.209.91.97:7778/api/StaticContent', { section: activeSection, content: contentData[activeSection] });
