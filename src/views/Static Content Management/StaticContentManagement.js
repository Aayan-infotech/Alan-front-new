...
const StaticContentManagement = () => {
  ...
  const fetchContent = async (section) => {
    try {
      const response = await axios.get(`https://www.discountdoorandwindow.com/api/StaticContent/${section}`);
  ...
  const saveContent = async () => {
    try {
      await axios.post('https://www.discountdoorandwindow.com/api/StaticContent', { section: activeSection, content: contentData[activeSection] });
