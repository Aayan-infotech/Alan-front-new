...
const PriceAdjustment = () => {
  ...
  useEffect(() => {
    if (filters.subCategory) {
      axios
        .get(`http://18.209.91.97:7778/api/subSubCategories/subcategoryid/${filters.subCategory}`)
  ...
  const applyPricing = async () => {
    try {
      const response = await axios.post('http://18.209.91.97:7778/api/updatePrices/PriceAdjustmentIncrease', {
  ...
  const applyPricingDecr = async () => {
    try {
      const response = await axios.post('http://18.209.91.97:7778/api/updatePrices/PriceAdjustmentDecrease', {
        category_id: filters.category,
        sub_category_id: filters.subCategory || undefined,
        sub_sub_category_id: filters.subSubCategory || undefined,
