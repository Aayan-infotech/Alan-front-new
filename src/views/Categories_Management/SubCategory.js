...
const SubCategory = () => {
  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      try {
        const response = await axios.get('http://54.236.98.193:7878/api/subcategory');
  ...
  const handleAddSubCategory = async () => {
    if (subCategoryName && selectedCategory && selectedImage) {
      ...
      try {
        await axios.post('http://54.236.98.193:7878/api/subcategory', formData, {
  ...
  const handleUpdateSubCategory = async () => {
    if (editSubCategory) {
      ...
      try {
        await axios.put(`http://54.236.98.193:7878/api/subcategory/${editSubCategory._id}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
  ...
  const handleDeleteSubCategory = async (subCategoryToDelete) => {
    try {
      await axios.delete(`http://54.236.98.193:7878/api/subcategory/${subCategoryToDelete._id}`);
  ...
  const handleToggleStatus = async (subCategory) => {
    try {
      ...
      await axios.put(`http://54.236.98.193:7878/api/subcategory/${subCategory._id}`, updatedSubCategory);
